...
function App() {
  ...
  const connectBlockChain = async () => {
    ...
    let deployedContract = new ethers.Contract(contract_address, Certification.abi, signer)
  ...
  const uploadCertificate = async (description) => {
    ...
    ipfs.files.add(buffer, async (error, result) => {
      ...
      const tx = await contract.uploadCertificate(result[0].hash,testOwner,description);
      console.log("tx = ", tx)
